...
def route(start, destination, currentstate):
	...
	encodePath(destination, currentstate)
...
def encodePath(destination, currentstate):
	...
	for i in range(1, len(destination.path)):
		...
		if instr != currentstate + 2 and instr != currentstate - 2:
